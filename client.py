...
class Client(tk.Frame):
    ...
    def send(self,event):
        ...
        if len(msg)==0:
            ...
            self.messageBox.insert(tk.INSERT, "Pesan tidak boleh kosong.")
        else:
    ...
    def on_closing(self):
        if tk.messagebox.askokcancel("Quit", "Apakah kamu mau keluar?"):
            self.exit(None)
        ...
        try:
            while self.status:
                ...
                try:
                    ...
                    if not echodane:
                        self.root.showMessage('','','Koneksi ke server gagal.','')
                        self.status=False
                        s.close()
                        break
                except:
